    ...
    m_SLAM(pSLAM)
{
    ...
    m_camera_position_publisher = this->create_publisher<PointMsg>("camera_position", 10);
...
void MonocularSlamNode::GrabImage(const ImageMsg::SharedPtr msg)
{
    ...
    if (!Tcw.empty() && mState == ORB_SLAM2::Tracking::OK)
    {
        std::ostringstream Tcw_stream;
        Tcw_stream << Tcw;
        RCLCPP_INFO(this->get_logger(), "Tcw: %s", Tcw_stream.str().c_str());
        ...
        camera_position.y = y;
        camera_position.z = z;
        m_camera_position_publisher->publish(camera_position);
        ...
        pos_stream << std::fixed << std::setprecision(3)
                   ...
                   << ", y=" << y
                   << ", z=" << z;
        RCLCPP_INFO(this->get_logger(), "%s", pos_stream.str().c_str());
